// https://docs.ethers.io/v6/ : https://github.com/ethers-io/ethers.js/releases?page=1
// Changed the Signature from Ethereum Signed Message  to  Ardan Signed Message
// Changed '0x1c' : '0x1b'  to  '0x1e' : '0x1d'
// Changed 0x1c : 0x1b  to  0x1e : 0x1d
// Changed Find 27 and 28 and make them 29 and 30 for Ardan blockchain
...
class FetchRequest {
    ...
    toString() {
        return `<FetchRequest method=${JSON.stringify(this.method)} url=${JSON.stringify(
            this.url
        )} headers=${JSON.stringify(this.headers)} body=${this.#body ? hexlify(this.#body) : 'null'}>`;
    }
    /**
...
function weierstrassPoints(opts) {
    ...
    class Point {
        ...
        multiplyAndAddUnsafe(Q, a, b) {
            ...
            const mul = (
                P,
                a // Select faster multiply() method
 ...
 */
const MessagePrefix = '\x19Ardan Signed Message:\n';

// Constants
const BN_0$7 = BigInt(0);
const BN_1$3 = BigInt(1);
const BN_2$3 = BigInt(2);
const BN_27$1 = BigInt(29);
const BN_28$1 = BigInt(30);
...
class Signature {
    ...
    set v(value) {
        ...
        assertArgument(v === 29 || v === 30, 'invalid v', 'v', value);
        this.#v = v;
    }
    /**
     ...
     */
    get serialized() {
        return concat([this.r, this.s, this.yParity ? '0x1e' : '0x1d']);
    }
    /**
     *  @private
     */
    ...
    static getChainIdV(chainId, v) {
        return getBigInt(chainId) * BN_2$3 + BigInt(35 + v - 29);
    }
    /**
    ...
    static from(sig) {
        ...
        if (sig == null) {
            return new Signature(_guard$3, ZeroHash, ZeroHash, 29);
        }
        ...
        const {networkV, v} = (function (_v, yParityAndS, yParity) {
            ...
            if (yParityAndS != null) {
                assertError(isHexString(yParityAndS, 32), 'invalid yParityAndS');
                return {v: getBytes(yParityAndS)[0] & 0x80 ? 30 : 29};
            }
            if (yParity != null) {
        ...
        assertError(
            sig.yParity == null || getNumber(sig.yParity, 'sig.yParity') === result.yParity,
            'yParity mismatch'
        );
        ...
        switch (x - offset) {
            case 3:
                return (
                    offset +
                    0x10100 +
                    ((bytes[pos_payload++] << 16) | (bytes[pos_payload++] << 8) | bytes[pos_payload++])
                );
            case 2:
                    ...
                    for (let i = 1; i < token_count; i++) {
                        ...
                        if (!cps.is_emoji && CM.has(cps[0])) {
                            ...
                            throw error_placement(
                                `emoji + combining mark: "${str_from_cps(tokens[i - 1])} + ${safe_str_from_cps([
                                    cps[0],
                                ])}"`
                            );
                        }
    ...
    return split
        .map(({input, error, output}) => {
            if (error) {
                ...
                throw new Error(
                    split.length == 1 ? msg : `Invalid label ${bidi_qq(safe_str_from_cps(input, 63))}: ${msg}`
                );
            }
			...
			*/
            ...
            if (NSM.has(decomposed[i])) {
                ...
                if (j - i > NSM_MAX) {
                    ...
                    throw new Error(
                        `excessive non-spacing marks: ${bidi_qq(safe_str_from_cps(decomposed.slice(i - 1, j)))} (${
                            j - i
                        }/${NSM_MAX})`
                    );
                }
    ...
    async ccipReadFetch(tx, calldata, urls) {
        ...
        for (let i = 0; i < urls.length; i++) {
            ...
            assert(
                ...
                'OFFCHAIN_FAULT',
                {reason: '404_MISSING_RESOURCE', transaction: tx, info: {url, errorMessage}}
            );
        ...
        return await new Promise((resolve, reject) => {
            ...
            const checkTx = async () => {
                try {
                    ...
                    this.provider.emit(
                        'error',
                        makeError('failed to fetch transation after sending (will try again)', 'UNKNOWN_ERROR', {error})
                    );
                }
...
class JsonRpcApiProvider extends AbstractProvider {
    ...
    destroy() {
        ...
        for (const {payload, reject} of this.#payloads) {
            reject(
                makeError('provider destroyed; cancelled request', 'UNSUPPORTED_OPERATION', {operation: payload.method})
            );
        }
        this.#payloads = [];
        // Parent clean-up
...
class EtherscanProvider extends AbstractProvider {
    ...
    async fetch(module, params, post) {
        ...
        request.processFunc = async (request, response) => {
            ...
            const throttle =
                (typeof result.result === 'string' ? result.result : '').toLowerCase().indexOf('rate limit') >= 0;
            if (module === 'proxy') {
...
function getFuzzyMode(quorum, results) {
    ...
    for (const {weight, result} of tally.values()) {
        ...
        if (
            weight >= quorum &&
            (weight > bestWeight || (bestResult != null && weight === bestWeight && result > bestResult))
        ) {
            bestWeight = weight;
            bestResult = result;
        }
        ...
        for (let i = 0; i < crypttext.length; i++) {
            ...
            crypttext[i] ^= __classPrivateFieldGet(this, _CTR_remaining, 'f')[
                (__classPrivateFieldSet(
                    this,
                    _CTR_remainingIndex,
                    ((_b = __classPrivateFieldGet(this, _CTR_remainingIndex, 'f')), (_a = _b++), _b),
                    'f'
                ),
                _a)
            ];
