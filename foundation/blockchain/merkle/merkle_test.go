...
func (t TestSHA256Content) Equals(other merkle.Data) (bool, error) {
...
// Equals tests for equality of two Contents.
func (t TestMD5Content) Equals(other merkle.Data) (bool, error) {
...
var table = []struct {
	...
	contents            []merkle.Data
	expectedHash        []byte
	notInContents       merkle.Data
}{
	{
		testCaseId:          0,
...
func TestNewTree(t *testing.T) {
	for i := 0; i < len(table); i++ {
		...
		tree, err := merkle.NewTree(table[i].contents)
		if err != nil {
...
func TestNewTreeWithHashingStrategy(t *testing.T) {
	for i := 0; i < len(table); i++ {
		tree, err := merkle.NewTree(table[i].contents, merkle.WithHashStrategy(table[i].hashStrategy))
		if err != nil {
			t.Errorf("[case:%d] error: unexpected error: %v", table[i].testCaseId, err)
		}
		if !bytes.Equal(tree.MerkleRoot, table[i].expectedHash) {
